---
title: "Spatial Epidemiology Using R"
author: "Dr Hazlienor"
date: "2024-10-21"
output: 
  html_document:
...
- disclaimer: the data used in this practice is simulated.
...
```{r}
pop_kel <- read_excel("pop_kel.xlsx")
```
...
# 5. Areal Data - Choropleth map

We will calculate the annual leptospirosis incidence for each subdistrict. First, we need to count the number of cases in each polygon.

```{r}
lep_in <- st_join(lepto, kel, 
                      join = st_within)

```

```{r}
count_lep_mukim <- lep_in %>% 
  count(DAERAH, MUKIM.x, tahun_daftar, jum2016, jum2017, jum2018, jum2019, jum2020, jum2021, jum2022) %>% 
  ungroup()

# Incidence Leptospirosis in 2016
count_lep_mukim_y16 <- subset(count_lep_mukim, tahun_daftar=='2016') 
count_lep_mukim_y16_1000 <- count_lep_mukim_y16 %>% 
  mutate(inc_1000 = (n/jum2016)*1000)
kelmap_with_lep_16 <- st_join(kel_map, count_lep_mukim_y16_1000)

# Incidence Leptospirosis in 2017
count_lep_mukim_y17 <- subset(count_lep_mukim, tahun_daftar=='2017') 
count_lep_mukim_y17_1000 <- count_lep_mukim_y17 %>% 
  mutate(inc_1000 = (n/jum2017)*1000)
kelmap_with_lep_17 <- st_join(kel_map, count_lep_mukim_y17_1000)

# Incidence Leptospirosis in 2018
count_lep_mukim_y18 <- subset(count_lep_mukim, tahun_daftar=='2018') 
count_lep_mukim_y18_1000 <- count_lep_mukim_y18 %>% 
  mutate(inc_1000 = (n/jum2018)*1000)
kelmap_with_lep_18 <- st_join(kel_map, count_lep_mukim_y18_1000)

# Incidence Leptospirosis in 2019
count_lep_mukim_y19 <- subset(count_lep_mukim, tahun_daftar=='2019') 
count_lep_mukim_y19_1000 <- count_lep_mukim_y19 %>% 
  mutate(inc_1000 = (n/jum2019)*1000)
kelmap_with_lep_19 <- st_join(kel_map, count_lep_mukim_y19_1000)

# Incidence Leptospirosis in 2020
count_lep_mukim_y20 <- subset(count_lep_mukim, tahun_daftar=='2020') 
count_lep_mukim_y20_1000 <- count_lep_mukim_y20 %>% 
  mutate(inc_1000 = (n/jum2020)*1000)
kelmap_with_lep_20 <- st_join(kel_map, count_lep_mukim_y20_1000)

# Incidence Leptospirosis in 2021
count_lep_mukim_y21 <- subset(count_lep_mukim, tahun_daftar=='2021') 
count_lep_mukim_y21_1000 <- count_lep_mukim_y21 %>% 
  mutate(inc_1000 = (n/jum2021)*1000)
kelmap_with_lep_21 <- st_join(kel_map, count_lep_mukim_y21_1000)

# Incidence Leptospirosis in 2022
count_lep_mukim_y22 <- subset(count_lep_mukim, tahun_daftar=='2022') 
count_lep_mukim_y22_1000 <- count_lep_mukim_y22 %>% 
  mutate(inc_1000 = (n/jum2022)*1000)
...
```{r}
# Find the common range of incidence values
common_range <- range(c(
  kelmap_with_lep_18$inc_1000,
  kelmap_with_lep_19$inc_1000,
  kelmap_with_lep_20$inc_1000,
  kelmap_with_lep_21$inc_1000,
  kelmap_with_lep_22$inc_1000
), na.rm = TRUE)


# Define common color scale
common_scale <- scale_fill_gradient(
  low = "lightblue",  # Lighter shade for lower values
  high = "#132B43",  # Darker shade for higher values
  limits = common_range,
  oob = scales::squish
)
...
grid.arrange(incplot_lep16, incplot_lep17, incplot_lep18, incplot_lep19, incplot_lep20, incplot_lep21, incplot_lep22, nrow=2, top=textGrob("Incidence Map of Leptospirosis in Kelantan per 1000 population 2016-2023", gp = gpar(fontsize = 20)))
...
Global Moran's I for leptospirosis 2016-2022
```{r}
polyLEP_kel <- merge(kel, kel_lepto2, by = c("MUKIM"))
count_lep <- polyLEP_kel%>% 
  count(DAERAH, MUKIM,jum2022 ) %>% 
  ungroup()
...
Local Moran's I for leptospirosis 2016-2022
```{r}
